...
通过轻量级的执行单元实现并发操作，主要依赖 `Goroutines`
，允许程序在并发执行过程中创建多个独立的执行流，他们可以相互协作和通信；每个 `Goroutines` 通常具有自己的执行状态，可以共享数据和通道

### 生成器模式

允许按需生成序列数据，而不是一次性生成所有数据；可以节约内存

### 反应器模式

事件驱动的设计模式，用于处理并发 `I/O` 操作，事件循环负责监听事件并根据事件类型调用相关的处理程序；处理程序可以是同步或者异步的
...
用于解决生产者和消费者之间的协作问题，生产者负责生产数据并放到共享内存中，消费者从队列中提取数据进行处理
...
- 消费者
    - 从队列中获取数据并进行处理
- 队列
    - 用于生产者和消费者之间的通讯，存放生产者产生的数据

### 批处理模式

用于处理大量数据和任务的情况，将多个操作合并成一个批次进行处理，以提高效率和性能；用于对大量数据进行操作或者需要批处理的任务场景

- 数据批次处理
    - 将多个数据或者任务合并成一个批次进行处理，减少处理次数
- 并发处理
    - 批处理模式常与并发处理结合
...
用于将多个输入通道中的数据合并到单个输入通道，并发的从每个输入通道读取数据，并将数据发送到输出通道

### 扇出模式

用于将单个输入通道中的数据分发到多个输出通道中，并发中通常将数据同时传递给多个处理任务，提高吞吐量和并发量。该模式能给有效的利用多核处理和并发

- 随机分发给单一输入源的数据，到不同的目的地
...
- `Future`
    - 未完成的操作或者值，充当异步计算结果的代理
- `Promise`
    - 用于接收 `Feature` 的计算结果，也可以设置 `Feature` 结果

### 发布订阅模式

多个发布者将消息发送给多个订阅者，该模式中发布者和订阅者是解耦的，发布者不直接发消息给订阅者，通过管道或者中介进行消息传递；可以动态的增加或者移除发布者和订阅者，同时减少组件之间的依赖。

### 推模式和拉模式

描述数据的传递方式，在 `Push` 模式下，数据的发布者直接推送给数据订阅者，`Pull` 模式下，订阅者主动从数据源中拉取数据